...
<template>
  <div
    id="container"
    class="w-full h-full overflow-hidden font-mono font-bold text-zinc-400 bg-slate-200"
  >
    <div
      :style="{
        transform:
          detailVisible == false ? 'translateX(0%)' : 'translateX(30%)',
      }"
      class="fixed flex flex-col w-5/12 px-4 pb-3 transition-transform duration-300 divide-y-4 divide-indigo-400 rounded-lg shadow-2xl bg-zinc-50 h-5/6"
    >
          ...
          <transition-group name="fade" tag="ul" class="h-96">
            ...
            >
              ...
              <div class="flex justify-around w-48">
                <button
                  v-if="editedId != item.userId && seeWhich != item.userId"
                ...
                <button
                  v-else-if="editedId == item.userId"
                  @click="editedId = -1"
                ...
                <button
                  v-else-if="seeWhich == item.userId"
                  @click="
                    () => {
                      detailVisible = false;
                      seeWhich = -1;
    ...
    <Detail :is-visible="detailVisible" :user-obj="cache" />
  </div>
</template>
